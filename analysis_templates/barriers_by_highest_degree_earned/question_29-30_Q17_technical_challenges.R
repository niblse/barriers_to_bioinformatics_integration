...
category.df <- data.frame ("Bachelors"= category.levels[1], 
                           ...
                           "Professional Degree" = category.levels[3] ,
                           "Doctoral" = category.levels[4], 
                           stringsAsFactors = FALSE)
...
sig.diff.chi.analysis <- function(df){
  ...
  proportional.sig.responses.summed.by.barriers.filename <- paste(table.dir.path,
                                                                  ...
                                                                  ".csv", 
                                                                  sep = "")
  write.csv(as.matrix(proportional.responses.summed.by.barriers.grouped.chi), 
            file = proportional.sig.responses.summed.by.barriers.filename)
