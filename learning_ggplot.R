...
# Gather question 38 super-category sums

q38_carnegie_analysis <- df%>%
...
q38_carnegie_analysis%>%
  filter(!is.na(Q21_What.is.the.Carnegie.classification.of.your.institution. ))%>%
  filter(q38_Faculty_issues_sum != "0")%>%
  ggplot()+
  aes(Q21_What.is.the.Carnegie.classification.of.your.institution.)+
  geom_bar()+
  stat_count(aes(label=..count..), vjust= -0.5, geom="text", position="identity")+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
#Generate a sum by carnegie catagory for each of the barrier super catagories
Assoc_var <- df%>%
  select(q38_Faculty_issues_sum, 
         q38_Curriculum_issues_sum, 
         q38_Resources_issues_sum, 
...
q21_38df <- data.frame(Assoc_var[1:8],
                       Bacca_var[1:8],
                       Doc_var[1:8],
                       Master_var[1:8])
...
q21_38df <- t(q21_38df)
q21_38df <- data.frame(q21_38df)

#melt the dataframe using reshape - do as matrix to preserve row names
melted_q21_38df <- melt(as.matrix(q21_38df))
...
ggsave("./plots/q38_barriers_by_carnegie_classification.png")
