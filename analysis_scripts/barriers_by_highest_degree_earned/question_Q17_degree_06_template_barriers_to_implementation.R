...
category.df <- data.frame ("Bachelors"= category.levels[1], 
                           "Masters" = category.levels[2], 
                           "Professional_Degree" = category.levels[3] ,
                           "Doctoral" = category.levels[4], 
                           stringsAsFactors = FALSE)

#Set an ordering for plotting - must match category.levels names
col.order <- c("1_B.S. (or equivalent)",
 "2_M.S. (or equivalent)",
 "3_Professional degree (e.g., M.D.)",
 "4_Ph.D. (or equivalent)" )

#With substitutions - must match category.df
col.order2 <- c("Bachelors", 
                "Masters" , 
                "Professional_Degree",
                "Doctoral")

#Nice Labels for plotting
nice.lables.list <- c("Bachelors", 
                "Masters" , 
                "Professional Degree",
                "Doctoral")
